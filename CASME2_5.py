...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str,
                        default='Cropped', help='Raf-DB dataset path.')
...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase, num_loso, transform=None, basic_aug=False, transform_norm=None):
        ...
        df = pd.read_excel(os.path.join(
            self.raf_path, 'CASME2-coding-20230315.xlsx'), usecols=[0, 1, 3, 4, 5, 7, 8])
    ...
    def __getitem__(self, idx):
        ...
        path_on0 = os.path.join(
            self.raf_path, sub, f, on0)
        path_on1 = os.path.join(
            self.raf_path, sub, f, on1)
        path_on2 = os.path.join(
            self.raf_path, sub, f, on2)
        path_on3 = os.path.join(
            self.raf_path,  sub, f, on3)
        path_apex0 = os.path.join(
            self.raf_path,  sub, f, apex0)
        path_off0 = os.path.join(
            self.raf_path,  sub, f, off0)
        path_off1 = os.path.join(
            self.raf_path,  sub, f, off1)
        path_off2 = os.path.join(
            self.raf_path,  sub, f, off2)
        path_off3 = os.path.join(
            self.raf_path,  sub, f, off3)
...
def run_training():
    ...
    for subj in LOSO:
        ...
        val_loader = torch.utils.data.DataLoader(val_dataset,
                                                 ...
                                                 prefetch_factor=2,
                                                 persistent_workers=True)
        # let train_loader and val_loader become cuda tensor
        ...
        # let all data in train_loader become cuda tensor
        ...
        torch.save(net_all.state_dict(), 'model_{}.pth'.format(subj))
