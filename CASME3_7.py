...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='/home/drink36/Desktop/apex_frame_take/casme3_result_2', help='Raf-DB dataset path.')
...
class RafDataSet(data.Dataset):
    def __init__(self, raf_path, phase,num_loso, transform = None, basic_aug = False, transform_norm=None):
        ...
        df = pd.read_csv("result_3.csv",usecols=[0,1,2,3,4,5,7])
        ...
        for (f,sub,onset,apex,offset,label_all,label_au) in zip(File_names,Subject,Onset_num,Apex_num,Offset_num,Label_all,Label_au):
            ...
            if label_all == 'happy' or label_all == 'disgust' or label_all == 'surprise' or label_all == 'others' or label_all == 'Others' or label_all=='anger' or label_all=='fear' or label_all=='sad':
    ...
    def __getitem__(self, idx):
        ...
        path_on1 = os.path.join(self.raf_path, sub, f, on1)
        path_on2 = os.path.join(self.raf_path, sub, f, on2)
        path_on3 = os.path.join(self.raf_path, sub, f, on3)
        path_apex0 = os.path.join(self.raf_path, sub, f, apex0)
        path_off0 = os.path.join(self.raf_path, sub, f, off0)
        path_off1 = os.path.join(self.raf_path, sub, f, off1)
        path_off2 = os.path.join(self.raf_path, sub, f, off2)
        path_off3 = os.path.join(self.raf_path, sub, f, off3)
